...
//     CPX a1 = (fft_direction == FftDirection::BACKWARD) ? exp(IMA*x[j]) : exp(-IMA*x[j]);
//     CPX a2 = (fft_direction == FftDirection::BACKWARD) ? exp(IMA*y[j]) : exp(-IMA*y[j]);
...
//     CPX ss = (fft_direction == FftDirection::BACKWARD) ? IMA*s[k] : -IMA*s[k];
//     CPX tt = (fft_direction == FftDirection::BACKWARD) ? IMA*t[k] : -IMA*t[k];
//     f[k] = CPX(0,0);
//     for (BIGINT j=0;j<nj;++j)
