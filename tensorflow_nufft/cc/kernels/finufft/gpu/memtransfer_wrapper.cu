		...
		case GpuSpreadMethod::NUPTS_DRIVEN:
			{
				if (d_plan->options_.gpu_sort_points) {
					int numbins[2];
					numbins[0] = ceil((FLT) nf1/d_plan->options_.gpu_bin_size.x);
					numbins[1] = ceil((FLT) nf2/d_plan->options_.gpu_bin_size.y);
		...
		case GpuSpreadMethod::NUPTS_DRIVEN:
			{
				if (d_plan->options_.gpu_sort_points) {
					...
					numbins[0] = ceil((FLT) nf1/d_plan->options_.gpu_bin_size.x);
					numbins[1] = ceil((FLT) nf2/d_plan->options_.gpu_bin_size.y);
					numbins[2] = ceil((FLT) nf3/d_plan->options_.gpu_bin_size.z);
					checkCudaErrors(cudaMalloc(&d_plan->binsize,numbins[0]*
		...
		case GpuSpreadMethod::BLOCK_GATHER:
			{
				...
				int binsperobins[3];
				numobins[0] = ceil((FLT) nf1/d_plan->options_.gpu_obin_size.x);
				numobins[1] = ceil((FLT) nf2/d_plan->options_.gpu_obin_size.y);
				numobins[2] = ceil((FLT) nf3/d_plan->options_.gpu_obin_size.z);
